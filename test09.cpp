...
TEST_CASE( "Test 09: stress test #4", "[Project07]" ) 
{
  //
  // StressTest #4:
  //
  ...
  // so we test random pushes, but here we have duplicate values.
  //
  int N = 500000;
  ...
  //
  // generate N random distances, with duplicates:
  //
  ...
  sort(randomPairs.begin(), randomPairs.end(),
    [](pair<int, int>& p1, pair<int, int>& p2)
    {
      if (p1.second < p2.second)  // sort by distance, ascending:
        ...
        return false;
      else  // distances are the same, so sort by vertex:
